    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function($query){
                $deleteBtn = "<a href='".route('admin.product-image-gallery.destroy', $query->id)."' class='btn btn-danger ml-2 delete-item'><i class='fa-solid fa-trash'></i></a>";
                return $deleteBtn;
            })
            ->addColumn('image', function($query){
                $img = "<img width='100px;' src='".asset($query->image)."'>";
