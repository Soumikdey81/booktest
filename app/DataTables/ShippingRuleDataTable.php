    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function($query){
                $editBtn = "<a href='".route('admin.shipping-rule.edit', $query->id)."' class='btn btn-primary'><i class='fa-solid fa-pen-to-square'></i></a>";
                $deleteBtn = "<a href='".route('admin.shipping-rule.destroy', $query->id)."' class='btn btn-danger ml-2 delete-item'><i class='fa-solid fa-trash'></i></a>";
            ...
            ->addColumn('min_cost', function($query){
                if($query->type == 'min_cost'){
                    return html_entity_decode('&#8377;').''.$query->min_cost;
                }else{
                    return html_entity_decode('&#8377;').''.'0';
                }
            })
            ->addColumn('cost', function($query){
                return html_entity_decode('&#8377;').''.$query->cost;
            })
